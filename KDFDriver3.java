...
public class KDFDriver3 {
	// Global Variable - Accessed anywhere in the class.
	WebDriver myD;
	String[][] xTC, xTS;
	int rowsTC;
	static int rowsTS;
	String xlPath,xlPath_Res;
	...
	public void getStarted() throws Exception{
		...
		xlPath_Res = "C:\\Users\\Jayashri\\eclipse-workspace\\KDF\\Test Data\\Keyword Driven Testing data sheet2 Result.xlsx";
		xTC = readXL(xlPath, "Test Cases");
		xTS = readXL(xlPath, "New Test Steps");
			...
			if (tcExecute.equals("Y")) {
				...
				for (int j = 1; j < rowsTS; j++) {
					...
					if (tcID.equals(tsID)) {
						...
						try {
							System.out.println("Executing on the AUT");
							executeKW();
							System.out.println("Executed on the AUT");
							tsResult = "Pass";
						} catch(Exception e) {
	...
	@After
	public void getCleaned() throws Exception {
		writeXL(xlPath_Res, "Test Steps", xTS);
	}
	...
	public void executeKW() {
		...
		if (tsKW.equals("navigateBrowser")) {
			navigateBrowser(tsTD);
		}
		if (tsKW.equals("clickLink")) {
			clickLink(tsEID);
		}
		if (tsKW.equals("clickElement")) {
			clickElement(tsEID);
		}
		if (tsKW.equals("verifyText")) {
			System.out.println(verifyText(tsEID, tsTD));
		}
		if (tsKW.equals("typeText")) {
			typeText(tsEID, tsTD);
		}
	}
	...
	// Method to read XL
			public static String[][] readXL(String fPath, String fSheet) throws Exception{
				...
				/*
				if (fSheet.equals("New Test Steps")) {
					rowsTS = xRows;
				}*/
